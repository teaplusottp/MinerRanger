...
// Build image URL (if server returns filename) /static/<file>
const toStaticUrl = (u) => {
  const s = String(u || "");
  if (!s) return s;
...
// Unwanted stats type: [{ activity_name|name|activity, count, percentage }, ...]
...
// Metric card: uses accent color (lighter than title), insights are even lighter
...
// Horizontal bar (each column is a solid color = group accent)
...
// Matrix table (use group accent for border/header/cell with data)
...
const UnwantedComboChart = ({ title, rows, accent }) => {
  ...
  // percentage can be 0-1 or 0-100
  const pctFraction = (p) => {
    if (p == null) return 0;
    const v = Number(p);
...
function GraphView() {
 ...
 const { selectedDb } = useDb()
  ...
  const renderProcessDiscovery = () => {
    ...
    // image (if there are fields with img_url)
    ...
    return (
      <Block>
        ...
        {/* BPMN Model from the bpmn_model field (DO NOT hardcode) */}
        {pd.bpmn_model && (
          <div
            style={{
              textAlign: "center",
  ...
  const renderPerformanceAnalysis = () => {
    ...
    // Matrix (the first field with data but no img_url)
